...
noise_levels <- c(0.5, 0.75, 1.0, 1.5, 2.0)
names(noise_levels) <- c("Very Low Noise", "Low Noise", "Moderate Noise", "High Noise", "Very High Noise")
...
#====================================================#
simulate_and_plot <- function(se_scale) {
  n_genes <- 18000
  n_traits <- 10
...
for (i in 1:length(noise_levels)) {
  setting_labels[i] <- sprintf("%s\n(SE scale factor = %.1f)", names(noise_levels)[i], noise_levels[i])
  print(setting_labels[i])
  plots <- simulate_and_plot(noise_levels[i])
...
pdf("noise_lambda_panel.pdf", width = 16, height = 14)
plot_grid(plotlist = lambda_plots, ncol = 5)
dev.off()

pdf("noise_rrmse_panel.pdf", width = 16, height = 14)
plot_grid(plotlist = rrmse_plots, ncol = 5, scale = 0.95)
dev.off()

pdf("noise_roc_panel.pdf", width = 16, height = 14)
plot_grid(plotlist = roc_plots, ncol = 5)
dev.off()
