...
eqtls <- get_significant_single_tissue_eqtls(gencodeIds = df$gencodeId, tissueSiteDetailIds = tissue, datasetId = "gtex_v10",
                                             itemsPerPage = 100000)
...
# Function to remove rows with GENPOS (b38) values within 500000 of each other for a given gene
remove_duplicates_within_500000_gene <- function(df, gene) {
  # Subset data for the given gene
  ...
  # Use a while loop instead of recursion to process the data
  i <- 1
...
remove_duplicates_within_500000 <- function(df) {
  # Process each gene separately and bind the results
...
compute_standard_error <- function(beta, pValue) {
  ...
  # Prevent extremely low p-values from causing infinite Z-scores
  adjusted_pValue <- pmax(pValue, 1e-300) 
  
  # Compute Z-score using lower.tail = TRUE
...
# Apply function to compute standard errors
eqtls_filtered$se <- mapply(compute_standard_error, eqtls_filtered$beta, eqtls_filtered$pValue)

# Remove unnecessary columns
eqtls_filtered <- eqtls_filtered[, c("snpId", "geneSymbol", "chr", "pos", "a1", "a2", "beta", "se", "tissueSiteDetailId")]
